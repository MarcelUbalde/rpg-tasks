...
function applyToUser(event, payload, user, rewardEventUserRepo, userRepo) {
...
// Main use case. CC=5.
export function applyRewardEventToUsers(
  { eventId, userIds },
  ...
  return transaction(() => {
    const event = rewardEventRepo.findById(eventId);
    ...
    for (const userId of uniqueIds) {
      ...
      results.push(applyToUser(event, payload, user, rewardEventUserRepo, userRepo));
    }
